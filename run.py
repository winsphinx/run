...
# 主函数
def run(user, passwd, step):
    user = str(user)
    password = str(passwd)
    step = str(step)
    if user == "" or password == "":
        print("用户名或密码填写有误！", user, password)
        return
...
if __name__ == "__main__":
    ...
    if len(user_list) == len(passwd_list):
        ...
        for line in range(0, len(user_list)):
            if len(step_range) == 2:
                step = str(random.randint(int(step_range[0]), int(step_range[1])))
            elif len(step_list):
                step = random.choice(step_list)

            push += run(user_list[line], passwd_list[line], step) + "\n"
    else:
        print("用户名和密码数量不一致")
